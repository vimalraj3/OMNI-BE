...
export class InvestmentController {
  ...
  private autoExecute() {
      ...
      try {
        ...
        for (const theInvestment of investments) {
          ...
          if (
            !investment ||
            investment.expired ||
            investment.type === InvestmentType.ACCOUNT_ACTIVATION ||
            investment.amountReturned >= investment.amount * 3
          ) {
            continue;
          }
    ...
    cron.schedule("0 9 * * 1", () => {
      this.investmentService.distributeBOAWeeklySalary();
    });
  }
  ...
  createInvestment = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      // ===================== CHECK BOA ===================== //
      await this.investmentService.calculateBOAGlobalBonus(amount);
      await this.investmentService.checkBOAQualification(user);
  ...
  activateAccount = catchAsync(
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      if (!amount || amount < 20)
        return next(
          new AppError("Account activation requires $20 deposit", 400)
        );
      ...
      if (user.accountActivated)
        return next(new AppError("Account already activated", 400));

      // ======================= ONE DOLLAR MAGIC ======================= //
      const oneDollarMagic = await this.investmentService.oneDollarMagic(user);
      if (!oneDollarMagic)
        return next(new AppError("Failed to activate account", 400));
      ...
      // ======================= SAVE NEW INVESTMENT ======================= //
      const saveNewInvestment = await this.investmentRepository.save(
        newInvestment
      );
